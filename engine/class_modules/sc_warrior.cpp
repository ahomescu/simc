...
public:
  ...
  struct talents_t
  {
    ...
    struct slayer_talents_t
    {
      ...
      player_talent_t violent_euphoria;  // NYI
      ...
      player_talent_t deadly_focus;  // NYI
...
struct demoralizing_shout_t : public warrior_attack_t
{
  demoralizing_shout_t( warrior_t* p, util::string_view options_str )
    : warrior_attack_t( "demoralizing_shout", p, p->talents.protection.demoralizing_shout )
  {
    parse_options( options_str );
    usable_while_channeling = true;
    aoe = -1;
...
struct storm_bolt_t : public warrior_attack_t
{
    ...
    : warrior_attack_t( "storm_bolt", p, p->talents.warrior.storm_bolt )
  {
    parse_options( options_str );
    may_dodge = may_parry = may_block = false;
    aoe = 1 + as<int>( data().effectN( 2 ).base_value() );
...
struct fury_whirlwind_parent_t : public warrior_attack_t
{
      ...
      additional_rage_gain_per_target( p->spec.whirlwind->effectN( 2 ).base_value() )
  {
    ...
    if ( p->main_hand_weapon.type != WEAPON_NONE )
    {
      mh_first_attack         = new whirlwind_fury_damage_t( "whirlwind_mh_first", p, data().effectN( 4 ).trigger() );
      mh_first_attack->weapon = &( p->main_hand_weapon );
      mh_first_attack->radius = radius;
      add_child( mh_first_attack );

      mh_other_attack         = new whirlwind_fury_damage_t( "whirlwind_mh_others", p, data().effectN( 6 ).trigger() );
      ...
      if ( p->off_hand_weapon.type != WEAPON_NONE )
      {
        ...
        oh_other_attack         = new whirlwind_fury_damage_t( "whirlwind_oh_others", p, data().effectN( 7 ).trigger() );
...
{
  whirlwind_arms_damage_t( util::string_view name, warrior_t* p, const spell_data_t* whirlwind )
    : warrior_attack_t( name, p, whirlwind )
  {
    aoe = -1;
    reduced_aoe_targets = 5.0;
...
struct arms_whirlwind_parent_t : public warrior_attack_t
{
      ...
      third_attack( nullptr )
  {
    ...
    if ( p->main_hand_weapon.type != WEAPON_NONE )
    {
      first_attack         = new whirlwind_arms_damage_t( "whirlwind_1", p, data().effectN( 1 ).trigger() );
...
void warrior_t::init_spells()
{
  ...
  talents.protection.deep_wounds            = find_talent_spell( talent_tree::SPECIALIZATION, "Deep Wounds", WARRIOR_PROTECTION );
  // Row 8
  ...
  talents.colossus.decimator                    = find_talent_spell( talent_tree::HERO, "Decimator" );
  ...
  talents.colossus.cut_to_the_bone              = find_talent_spell( talent_tree::HERO, "Cut to the Bone" );
  ...
  talents.colossus.celeritous_conclusion        = find_talent_spell( talent_tree::HERO, "Celeritous Conclusion" );
  ...
  talents.slayer.violent_euphoria      = find_talent_spell( talent_tree::HERO, "Violent Euphoria" );
  ...
  talents.slayer.deadly_focus          = find_talent_spell( talent_tree::HERO, "Deadly Focus" );
  ...
  talents.slayer.unhinged              = find_talent_spell( talent_tree::HERO, "Unhinged" );
  ...
  talents.mountain_thane.storm_surge                  = find_talent_spell( talent_tree::HERO, "Storm Surge" );
  ...
  talents.mountain_thane.conductivity                 = find_talent_spell( talent_tree::HERO, "Conductivity" );
  ...
  talents.mountain_thane.capacitance                  = find_talent_spell( talent_tree::HERO, "Capacitance" );
...
void warrior_t::combat_begin()
{
  ...
  if ( talents.shared.bloodsurge->ok() )
  {
